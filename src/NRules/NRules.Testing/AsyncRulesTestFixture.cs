...
/// <summary>
/// Fixture to test rules using an async session.
/// </summary>
public class AsyncRulesTestFixture
{
    private readonly Lazy<AsyncRulesTestHarness> _testHarness;
    private IRuleAsserter? _asserter;

    /// <summary>
    /// Initializes a new instance of the <see cref="AsyncRulesTestFixture"/> class.
    /// </summary>
    public AsyncRulesTestFixture()
    {
        Setup = new RulesTestSetup();
        _testHarness = new Lazy<AsyncRulesTestHarness>(() => new AsyncRulesTestHarness(Setup));
    }
    ...
    /// <summary>
    /// Gets the current async rules engine session.
    /// </summary>
    public IAsyncSession Session => _testHarness.Value.Session;
